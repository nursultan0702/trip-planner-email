...
class EmailServiceImplTest {
  ...
  void testSendHtmlMessage() {
    ...
    EmailServiceImpl service = new EmailServiceImpl(mockMailSender, mockTemplateEngine, mockUserService);
    MimeMessage mockMimeMessage = mock(MimeMessage.class);

    when(mockUserService.getUserByEmail("john.doe@mail.com")).thenReturn(user);
    when(mockTemplateEngine.process(anyString(), any(Context.class))).thenReturn("<html>Email Content</html>");
  ...
  void testGenerateGoogleCalendarLink() {
    ...
    String link = service.generateGoogleCalendarLink("Trip", start, end, "Trip USA", locations);
